...
Public Class frmListingWarehouseMovement
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        If cmbOption.SelectedIndex = 0 Then 'header
            ...
            table = GetMovementDetail(Trim(GridHeader.SelectedCells(0).Value), cmbStatus.SelectedIndex)
        Else 'detail
            table = GetMovementDetail(Trim(gridDetail.SelectedCells(0).Value), cmbStatus.SelectedIndex)
        End If
        ...
        params.Add(param)
        param = New ReportParameter("reffParam", table.Rows(0).Item(2).ToString)
        params.Add(param)

        If cmbStatus.SelectedIndex = 0 Then
            stsMovement = IIf(table.Rows(0).Item(3).ToString = "PN102", "Purchase Return Consignment", "Purchase Return Credit")
        ElseIf cmbStatus.SelectedIndex = 1 Then
            stsMovement = "Transfer Out - Interbranch"
        Else
            stsMovement = "Inter Warehouse Movement"
        End If

        param = New ReportParameter("transIdParam", stsMovement)
        params.Add(param)
        param = New ReportParameter("suppNameParam", table.Rows(0).Item(4).ToString)
        params.Add(param)
        param = New ReportParameter("addressParam", table.Rows(0).Item(5).ToString)
        params.Add(param)
        param = New ReportParameter("noteParam", table.Rows(0).Item(10).ToString)
        params.Add(param)

        Me.Cursor = Cursors.Default

        frmReportViewer.Datasource = ds
        frmReportViewer.ReportDocument = "SEE_POS_OFFLINE.MovementForm.rdlc"
        frmReportViewer.ReportName = "MovementForm"
        frmReportViewer.ReportDatasetName = "Movement_Dataset"
