...
#Region "Reporting"
    ...
    Public Shared Function ClipboardItemWarehouseStockLevel(ByVal data As DataTable, opt As Integer, wh As String) As DataTable
        dtTable = New DataTable
    ...
    Public Shared Function GetDetailItemPOS(ByVal kode As String, ByVal discgroup As String, ByVal pricegroup As String) As DataTable
        ...
        Try
            ...
            query = "Select TOP 1 type_partnumber,type_description,type_uom,Param_D1,Disc1 Disc1_Rate,Param_D2,Disc2 Disc2_Rate," &
                                ...
                                "ISNULL(type_prodhier3,'')type_prodhier3,ISNULL(type_prodhier4,'')type_prodhier4,ISNULL(type_prodhier5,'')type_prodhier5,type_discgroup from " & DB & ".dbo.mtipe with (nolock) " &
                                ...
                                " inner join " & DB & ".dbo.mprice with (nolock) on type_partnumber=mp_partnumber" &
                                " inner join " & DB & ".dbo.mctprod with (nolock) on product_code=type_product" &
    ...
    Public Shared Function RptListingWarehouseReceive(ByVal kode As String, fromDt As Date, toDt As Date,
        ...
        Try
            ...
            Select Case state
                Case 0
                    query = "SELECT hlbm_wrs docno,(SELECT DISTINCT dlbm_reffdoc FROM " & DB & ".dbo.twrsd with(nolock)" &
                            ...
                            "ELSE 'Draft' END sts FROM " & DB & ".dbo.twrsh with(nolock) where hlbm_date BETWEEN '" & Format(fromDt, formatDate) & "' AND '" & Format(toDt, formatDate) & "' " &
        ...
        Try
            ...
            If GetValueParamNumber("SYSTEM SQL") = 1 Then
                query = "SELECT LTRIM(RTRIM(type_partnumber)) Item,LTRIM(RTRIM(type_description)) Judul," &
                        ...
                        "(SELECT CAST(ISNULL(SUM(ds_qty),0) AS INTEGER) FROM " & DB & ".dbo.TSLSD WITH(NOLOCK) " &
                        ...
                        "CAST(part_rfsstock AS INTEGER) Stock,(select TOP 1 hlbm_wrs FROM " & DB & ".dbo.TWRSD WITH(NOLOCK) " &
                        "INNER JOIN " & DB & ".dbo.TWRSH WITH(NOLOCK) on HLBM_WRS=DLBM_WRS " &
                        "WHERE DLBM_PartNumber = TYPE_PartNumber " &
                        "AND HLBM_TrnID in ('GR102','GR101','GR410') " &
                        "ORDER BY HLBM_Date desc )Last_BM," &
                        "(SELECT TOP 1 hlbm_date FROM " & DB & ".dbo.TWRSD WITH(NOLOCK) " &
                        "INNER JOIN " & DB & ".dbo.TWRSH WITH(NOLOCK) on HLBM_WRS=DLBM_WRS " &
                        "WHERE DLBM_PartNumber = TYPE_PartNumber " &
                        "AND HLBM_TrnID in ('GR102','GR101','GR410') " &
                        "ORDER BY HLBM_Date desc )BM_Date," &
                        "(SELECT top 1 CAST(DLBM_StockQty AS INTEGER) FROM " & DB & ".dbo.TWRSD WITH(NOLOCK) " &
                        "INNER JOIN " & DB & ".dbo.TWRSH WITH(NOLOCK) on HLBM_WRS=DLBM_WRS " &
                        "WHERE DLBM_PartNumber = TYPE_PartNumber " &
                        "AND HLBM_TrnID in ('GR102','GR101','GR410') " &
                        "ORDER BY HLBM_Date desc )BM_Qty," &
                        "(SELECT TOP 1 hlbm_dn FROM " & DB & ".dbo.TWRSD WITH(NOLOCK) " &
                        "INNER JOIN " & DB & ".dbo.TWRSH WITH(NOLOCK) on HLBM_WRS=DLBM_WRS " &
                        "WHERE DLBM_PartNumber = TYPE_PartNumber " &
                        "AND HLBM_TrnID in ('GR102','GR101','GR410') " &
                        "ORDER BY HLBM_Date desc )DN," &
                        "ISNULL((SELECT TOP 1 TMatReqd.requestno FROM " & DB & ".dbo.TMatReqd WITH(NOLOCK) " &
                        "INNER JOIN " & DB & ".dbo.TMatReqH WITH(NOLOCK) ON tmatreqh.Requestno=TMatreqD.Requestno " &
                        "WHERE Partnumber = TYPE_PartNumber AND Warehouse= '" & wh & "' " &
                        "ORDER BY DocumentDate ),'')MR FROM " & DB & ".dbo.MTIPE WITH(NOLOCK) " &
                        "INNER JOIN " & DB & ".dbo.MCTPROD WITH(NOLOCK) on PRODUCT_Code=TYPE_Product " &
                        "INNER JOIN " & DB & ".dbo.MPART WITH(NOLOCK) on part_partnumber=type_partnumber and PART_WH='" & wh & "' "
    ...
    Public Shared Function ReportInventoryDemand(ByVal period As Date, ByVal group As String, ByVal sts As String, _
        ...
        Try
            ...
            With cm
                ...
                .CommandText = "EXECUTE " & DB & ".dbo.P_INVENTORY_DEMAND '" & Format(period, formatDate) & "','" & Trim(group) & "'," &
            ...
            If GetValueParamText("SYSTEM SQL") = 1 Then
                ...
                query = query + "INSERT INTO @tmp " &
                    "SELECT " &
                     ...
                     "INNER JOIN " & DB & ".dbo.mtipe with(nolock) on type_partnumber=part_partnumber " &
                     "INNER JOIN " & DB & ".dbo.mmca with(nolock) on type_materialtype=mat_tipe " &
                     "INNER JOIN " & DB & ".dbo.mctprod with(nolock) on type_product=product_code " &
                     "INNER JOIN " & DB & ".dbo.mpdisc with(nolock) on type_discgroup=product WHERE type_prodhier5='" & supp & "' "
    ...
    Public Shared Function ListingSalesHeader(ByVal emp As String, ByVal dtFrom As Date,
    ...
    Public Shared Function ListingSalesAll(ByVal org As String, ByVal dtFrom As Date,
        ...
        Try
            ...
            query = "SELECT hs_invoice invoice,hs_invoicedate date,hs_customer customer," &
                    ...
                    "FROM " & DB & ".dbo.tslsh WITH (NOLOCK) " &
                    "INNER JOIN " & DB & ".dbo.mwh WITH (NOLOCK) on wh_kode=hs_warehouse " &
                    "WHERE hs_invoicedate between '" & Format(dtFrom, formatDate) & "' " &
                    "AND '" & Format(dtTo, formatDate) & "' " &
                    "AND hs_salesorg='" & org & "'"
    ...
    Public Shared Function ClosingCashier(ByVal dtFrom As Date,
        ...
        Try
            ...
            query = "SELECT cashierbal_id,cashierbal_userid,cashierbal_opendate" &
                            ...
                            "FROM " & DB & ".dbo.cashierbalance with(nolock) " &
    ...
    Public Shared Function ListingReceiveDetail(ByVal kode As String, fromDt As Date, toDt As Date,
                                                      ByVal state As Integer) As DataTable
        Try
            ...
            Select Case state
                Case 0
                    query = "SELECT LTRIM(RTRIM(hlbm_wrs)) docno,(SELECT DISTINCT LTRIM(RTRIM(dlbm_reffdoc)) FROM " & DB & ".dbo.twrsd with(nolock)" &
                            ...
                            "FROM " & DB & ".dbo.twrsh with(nolock) " &
                            "INNER JOIN " & DB & ".dbo.twrsd with(nolock) ON dlbm_wrs=hlbm_wrs " &
                            "INNER JOIN " & DB & ".dbo.mtipe with(nolock) ON dlbm_partnumber=type_partnumber " &
                            "WHERE hlbm_date BETWEEN '" & Format(fromDt, formatDate) & "' AND '" & Format(toDt, formatDate) & "' " &
                            "AND hlbm_trnid IN ('GR102','GR101')"
                Case 1
                    query = "SELECT LTRIM(RTRIM(hlbm_wrs)) docno,(SELECT DISTINCT LTRIM(RTRIM(dlbm_reffdoc)) FROM " & DB & ".dbo.twrsd with(nolock)" &
                            " WHERE dlbm_wrs=hlbm_wrs)AS pono,hlbm_date docdate,LTRIM(RTRIM(dlbm_partnumber)) item," &
        ...
        Try
            ...
            With cm
                ...
                .CommandText = "SELECT ROW_NUMBER() OVER(ORDER BY type_description) iden,RTRIM(LTRIM(ds_partnumber)) Item," &
                                ...
                                "FROM " & DB & ".dbo.tslsd " &
                                "INNER JOIN " & DB & ".dbo.mtipe on type_partnumber=ds_partnumber " &
    ...
    Public Shared Function GetMovementDetail(ByVal no As String, sts As Integer) As DataTable
        ...
        Try
            ...
            If sts = 0 Then 'return supplier
                query = "SELECT dts_doi document,hts_date date,hts_reffdoc reffdoc,hts_trnid transid," &
                  ...
                  "FROM " & DB & ".dbo.ttsd " &
                  "INNER JOIN " & DB & ".dbo.ttsh ON hts_doi=dts_doi " &
                  "INNER JOIN " & DB & ".dbo.mspl ON sup_supplier=hts_supplier " &
                  "INNER JOIN " & DB & ".dbo.mtipe ON type_partnumber=dts_partnumber where dts_doi='" & no & "'"
            ElseIf sts = 1 Then 'interbranch
                query = "SELECT dts_doi document,hts_date date,hts_reffdoc reffdoc,hts_trnid transid," &
                 ...
                 "FROM " & DB & ".dbo.ttsd " &
                 "INNER JOIN " & DB & ".dbo.ttsh ON hts_doi=dts_doi " &
                 "INNER JOIN " & DB & ".dbo.mcust ON cust_kode=hts_customer " &
                 "INNER JOIN " & DB & ".dbo.mtipe ON type_partnumber=dts_partnumber where dts_doi='" & no & "'"
            Else
                query = "SELECT dts_doi document,hts_date date,hts_reffdoc reffdoc,hts_trnid transid," &
                ...
                "FROM " & DB & ".dbo.ttsd " &
                "INNER JOIN " & DB & ".dbo.ttsh ON hts_doi=dts_doi " &
                "INNER JOIN " & DB & ".dbo.mwh ON wh_kode=hts_to_wh " &
                "INNER JOIN " & DB & ".dbo.mtipe ON type_partnumber=dts_partnumber where dts_doi='" & no & "'"
            End If
    ...
    Public Shared Function GetReceiveDetail(ByVal no As String, sts As Integer) As DataTable
        ...
        Try
            ...
            If sts = 0 Then 'Goods Receive
                query = "SELECT dlbm_wrs document,hlbm_date date,dlbm_reffdoc reffdoc,hlbm_dn dn,hlbm_dndate dndate,hlbm_trnid transid," &
                  ...
                  "FROM " & DB & ".dbo.twrsd " &
                  "INNER JOIN " & DB & ".dbo.twrsh ON hlbm_wrs=dlbm_wrs " &
                  "INNER JOIN " & DB & ".dbo.mspl ON sup_supplier=hlbm_supplier " &
                  "INNER JOIN " & DB & ".dbo.mtipe ON type_partnumber=dlbm_partnumber where dlbm_wrs='" & no & "'"
            ElseIf sts = 1 Then 'interbranch
                query = "SELECT dts_doi document,hts_date date,hts_reffdoc reffdoc,hts_trnid transid," &
                 "LTRIM(RTRIM(cust_nama)) customer,cust_address_1 address,LTRIM(RTRIM(dts_partnumber)) partnumber," &
                 "LTRIM(RTRIM(type_description)) description,dts_uom uom,dts_qty qty,hts_note note,type_spl_material1 vendor " &
...
#Region "Inventory"
        ...
        Try
            ...
            If state = 0 Then
                query = "SELECT promoid,description,note,disctype,assignto,periodfrom,periodto," &
                        "prodhier,validflag,closeflag,autogenerate,minpayment,createuser,createdate FROM " & DB & ".dbo.mdisch " &
    ...
    Public Shared Function GetEventHeader(ByVal no As String) As DataTable
...
#Region "Display"
    Public Shared Sub InsertDisplayRack(ByVal rackNo As String, ByVal item As String)
        Try
    ...
    Public Shared Sub InsertDisplayItem(ByVal rackNo As String, ByVal item As String)
        Try
